...
reg [2:0] wr_pixel_c = 3'd0;
reg [3:0] wr_en_lb = 4'b0000;
reg [1:0] wr_buffer = 2'b00;
reg [5:0] total_wr_pixel = 6'd0;
reg wr_en = 1'b0 ;
reg rd_en = 1'b0 ;
reg [2:0] line_c = 3'd0;
reg [2:0] rd_pixel_c = 2'd0;
reg [3:0] rd_en_lb = 4'b0000;
reg [1:0] rd_buffer = 2'b00;
reg [5:0] total_rd_pixel = 6'd0;
reg [5:0] total_rd_pixel_after_cov = 6'd0;
...
if (rd_pixel_c == 3'd7) begin
rd_buffer <= rd_buffer +1;
rd_pixel_c <= 3'd0;
if (line_c == 3'd7) begin
line_c <= 3'd0;
end
else begin
line_c <= line_c +1;
...
always @(*)
begin
if(line_c== 3'd0) begin
data_out = {lb1_data,lb0_data,24'd0};
end
else if (line_c == 3'd7) begin
case(rd_buffer)
0:begin
...
always @(posedge clk) begin
if (rst) begin
total_rd_pixel_after_cov <= 6'd0;
end
else if (total_rd_pixel_after_cov == 6'd63) begin
total_rd_pixel_after_cov <= 6'd0;
end
...
always @(*) begin
if (total_rd_pixel_after_cov == 6'd63) begin
last_bit <= 1'b1;
end
else begin
last_bit <= 1'b0;
